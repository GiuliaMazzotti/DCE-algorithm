...
%   directional distance to canopy edge (DCE), following concepts 
%   introduced in the manuscript:
%   Mazzotti et al., 'Revisiting Snow Cover Variability and Canopy 
%   Structure within Forest Stands: Insights from Airborne Lidar Data', 
%   submitted to Water Resources Research in January 2019.
%   The script includes 
...
%   wrapper.
%   This script requires a canopy height model in ASCII format; 
...
%   Note also that some NaN values are generated at the edge of the CHM 
%   during the smoothing procedure. The input CHM should therefore include  
%   a buffer.
% 
% OUTPUT
%   DCE grids in ascii format are saved in the user-defined directory.
%
% USAGE
%   Provide user input
...
% Filename of input CHM grid in ascii format
chm_infile = 'chm_test.asc'; 
% Output DCE grids filenames (can be left empty if not computed)
...
% height cutoff of 2m is recommended and commonly used in literature
height_cut = 2;
...
if strcmpi(mode,'all') || strcmpi(mode,'simple')
  save_ascii_grid(fullfile(outfolder,dce_outfile),'dce');
end
if strcmpi(mode,'all') || strcmpi(mode,'directional')
  save_ascii_grid(fullfile(outfolder,ndce_outfile),'ndce');
  save_ascii_grid(fullfile(outfolder,sdce_outfile),'sdce');
end 
% end of wrapper
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [dce_output, ndce_output, sdce_output] = calcDCE(chm_input,mode,step_nr)
  ...
  chm_data = chm_input; 
  chm_data.ncols = chm_input.ncols*chm_input.cellsize;
  chm_data.nrows = chm_input.nrows*chm_input.cellsize;
  chm_data.cellsize = 1; 
  ...
  if strcmpi(mode,'all') || strcmpi(mode,'simple')
    ...
    dceall_grid = opnclasses-canclasses;
    ...
    dce_output.data = imresize(dceall_grid.*chm_input.cellsize,1/chm_input.cellsize,'bilinear');
    % visualize
  ...
  if strcmpi(mode,'all') || strcmpi(mode,'directional')
    ...
    figure; imagesc(ndceall_grid); 
    ndce_output = chm_input; 
    ndce_output.data = imresize(ndceall_grid.*chm_input.cellsize,1/chm_input.cellsize,'bilinear');
    ...
    figure; imagesc(sdceall_grid); 
    sdce_output = chm_input; 
    sdce_output.data = imresize(sdceall_grid.*chm_input.cellsize,1/chm_input.cellsize,'bilinear');
  end
