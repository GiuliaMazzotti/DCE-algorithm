...
% 0 = open pixels. 
chm_infile = 'Laret_chm_mosaic.tif'; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CALCULATIONS
% 0. identify input file format
[filepath,name,ext] = fileparts(fullfile(infolder,chm_infile)); 

% 1. load CHM data
if contains(ext,'asc')
  chm_grid = load_ascii_grid(fullfile(infolder,chm_infile));
elseif contains(ext,'tif')
  chm_grid = read_convert_tif(fullfile(infolder,chm_infile));
end 
...
function answer = load_ascii_grid(varargin)
  ...
  %   (number of rows), xll and yllcorners (lower left coordinates),
  %   cellsize (grid cell resolution), NODATA_value
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SUBFUNCTION: IMPORT TIF AND CONVERT TO GRID
function answer = read_convert_tif(tiffile)
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % DESCRIPTION:
  %   loads grid in TIF format and converts to sth equivalent to ascii
  %
  % IMPLEMENTATION:
  %   by GM in November-2021 @ SLF Switzerland
  %
  % INPUT:
  %   tif file name
  % 
  ...
  [griddata,gridinfo] = readgeoraster(tiffile); 
  
  % some checks
  if gridinfo.CellExtentInWorldX ~= gridinfo.CellExtentInWorldY
    error('non-square cell size of input grid. algorithm not capable of handling this case yet');
  end 
  
  grid.nrows = gridinfo.RasterSize(1);
  grid.ncols = gridinfo.RasterSize(2);
  grid.xllcorner = gridinfo.XWorldLimits(1);
  grid.yllcorner = gridinfo.YWorldLimits(1);
  grid.cellsize = gridinfo.CellExtentInWorldX;
  grid.NODATA_value = min(min(griddata));
  grid.data = griddata; 
